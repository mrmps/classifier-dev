...
export async function requireApiAccount(request: Request, env: AppEnv): Promise<string> {
  if (!isLocalDemo(request, env) && env.APP_ACCOUNTS_ENABLED !== "true")
    throw new AppError(503, "Account credentials are not enabled for this deployment.");
  const match = /^Bearer\s+(classifier_agent_[A-Za-z0-9_-]+)$/i.exec(request.headers.get("authorization") ?? "");
  ...
  ).bind(await hashToken(match[1])).first<{ account_id: string }>();
  if (!key) throw new AppError(401, "Invalid, paused, or revoked API key.");
  if (!isLocalDemo(request, env) && await isDemoWorkspaceRecord(key.account_id, env))
    throw new AppError(401, "Local credentials cannot authorize deployed requests.");
  return key.account_id;
}

function dollars(credits: number): string {
  if (!Number.isSafeInteger(credits) || credits < 0) throw new AppError(503, "Balance is unavailable.");
...
export async function readAccountBalance(accountId: string, env: AppEnv) {
  ...
  ).bind(accountId).first<{ balance: number; reserved: number }>();
  if (!row) throw new AppError(404, "Account not found.");
  return { currency: "USD", available: dollars(row.balance), reserved: dollars(row.reserved), exact: true };
}
