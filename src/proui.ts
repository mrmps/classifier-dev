...
.pro input{font:inherit;padding:10px 12px;border:1px solid var(--line);border-radius:var(--r);background:var(--surface);color:var(--fg);max-width:420px;width:100%;box-sizing:border-box}
.pro button,.pro a.primary{min-height:40px}.pro .primary{background:var(--accent);color:var(--ink);padding:8px 16px;justify-content:center}.pro button:disabled{opacity:.5;cursor:wait}
...
<section id="signin" hidden><h2>Sign in to get Pro</h2><p>Use your account to subscribe, manage billing and API keys.</p><p><a class="b primary" href="/v1/billing/login">Sign in</a></p></section>
...
(() => {
  ...
  $('logout').onclick = () => run($('logout'), async () => { const result = await api('logout', {}); signedOut(); location.assign(result.url); });
  ...
  refresh().catch(error => { signedOut(); message(error.message, true); });
})();
</script>`,
  });
}
