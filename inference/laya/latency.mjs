...
if (!output) throw new Error('Usage: node inference/laya/latency.mjs <output.json>');
const rows = [];
for (let i = 0; i < 20; i++) {
  const started = performance.now();
  const response = await fetch('https://classifier.dev/v1/classify', {
    method:'POST', headers:{'content-type':'application/json'},
    body:JSON.stringify({model:'laya',processing:'fast',input:'Please refund this charge',labels:['billing','technical']}),
  });
...
const summary = Object.fromEntries(['ms','worker_total','quota_regular','quota_laya','modal_fetch','backend'].map(key=>[key,{p50:percentile(key,.5),p95:percentile(key,.95)}]));
writeFileSync(output,JSON.stringify({at:new Date().toISOString(),conditions:'20 sequential public anonymous fast-lane calls from the same local client; first call reported separately; 300ms between requests',first:rows[0],summary,rows},null,2));
