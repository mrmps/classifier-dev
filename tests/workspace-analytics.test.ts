...
test("analytics authorize the explicitly requested workspace and reject non-members before fetching", async () => {
  ...
  for (const id of ["person", "team", "unrelated"]) {
    ...
    await db.prepare("INSERT INTO app_workspaces(account_id,kind,mode,created_at) VALUES(?,'organization','hosted','now')").bind(id).run();
  }
  await db.prepare("INSERT INTO app_memberships(identity_account_id,workspace_id,role,joined_at) VALUES('person','team','member','now')").run();
  let calls = 0;
  const fetcher = (async (_url, init) => {
    calls++;
    expect(init?.body).toContain("index1 = 'team'");
    expect(init?.body).not.toContain("index1 = 'person'");
    return Response.json({ data: [] });
  }) as typeof fetch;
  await readWorkspaceAnalytics("person", { workspaceId: "team", kind: "summary", params: {} }, env, fetcher);
  expect(calls).toBe(1);
  ...
  for (const params of ["x", [], { interval: 1 }]) {
    await expect(readWorkspaceAnalytics("person", { workspaceId: "team", kind: "summary", params } as never, env, fetcher)).rejects.toThrow("Invalid analytics query");
  }
  expect(calls).toBe(1);
});
