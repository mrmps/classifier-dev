...
test("sign-out converts AuthKit redirects to GET navigation and clears both session cookies", () => {
  const result = spawnSync(process.execPath, ["-e", `
    ...
    assert.match(response.headers.get("Set-Cookie"), /wos-session=;[^,]*Max-Age=0/);
    ...
    assert.match(expired.headers.get("Set-Cookie"), /classifier_workspace=;[^,]*Max-Age=0/);
    await post({request:new Request("https://preview.example.test/auth/sign-out", {method:"POST",headers:{Origin:"https://preview.example.test"}})});
